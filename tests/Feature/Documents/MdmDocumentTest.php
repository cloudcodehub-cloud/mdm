...
class MdmDocumentTest extends TestCase
{
    ...
    public function test_framework_uses_agency_timezone_and_generated_by_identity(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ->component('documents/viewer')
                ->where('title', 'Client Visit & Task Handout')
                ->where('downloadUrl', route('documents.visit-handout.pdf', $scheduled))
            )
            ->assertSee('Harbor Light Supports', false)
            ->assertSee('Client Visit & Task Handout', false)
    ...
    public function test_completed_visit_report_preserves_task_outcomes_and_clock_history(): void
    {
        ...
        $preview->assertSee('Completed Visit Report', false)
            ->assertSee('Morning ADLs', false)
    ...
    public function test_hours_report_respects_scope_filters_and_omits_payroll_fields(): void
    {
        ...
        $adminPreview->assertInertia(fn (Assert $page) => $page
            ->component('documents/viewer')
            ->where('title', 'Employee Hours & Attendance Report')
        );
        $adminPreview->assertSee($world['dsp']->full_name, false)
        ...
        $this->actingAs($world['supervisor']->user()->firstOrFail())
            ...
            ->assertOk()
            ->assertInertia(fn (Assert $page) => $page
                ->where('html', fn (string $html): bool => str_contains($html, $world['dsp']->full_name)
                    && ! str_contains($html, $otherDsp->full_name))
            );
